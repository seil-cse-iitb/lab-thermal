...
con = mysql.connect()
cur = con.cursor()
# End config for 2nd app

# Second mysql db conn to seil sensor data to fetch temperature
# datamysql = MySQL()
# app.config['MYSQL_DATABASE_USER'] = 'reader'
# app.config['MYSQL_DATABASE_PASSWORD'] = 'reader'
# app.config['MYSQL_DATABASE_DB'] = 'seil_sensor_data'
# app.config['MYSQL_DATABASE_HOST'] = 'mysql.seil.cse.iitb.ac.in'
# datamysql.init_app(app)
# datacon = datamysql.connect()
# datacur = datacon.cursor()

datacon = mysql_connector.connect(host='mysql.seil.cse.iitb.ac.in',user='reader',passwd='reader',database='seil_sensor_data')
datacur = datacon.cursor()

# Config for 2nd mysql done
...
def getSeatData(i):
    ...
    query = "select temperature from dht_7 where id = '%d' and sensor_id='temp_k_seil' order by TS desc limit 1" %(i)
    datacur.execute(query)
    res = datacur.fetchone()
    # con.close()
    #print res[0]
    return res[0]
...
def makeTempMatrix(temp):
    ...
    print("**********************"+str(len(temp))+"**************")
    tempMatrix[0][0]=temp[3]
    tempMatrix[0][1]=temp[1]
    tempMatrix[0][2]=temp[1]

    tempMatrix[1][0]=temp[3]
    tempMatrix[1][1]=temp[1]
    tempMatrix[1][2]=temp[1]

    tempMatrix[2][0]=temp[2]
    tempMatrix[2][1]=temp[2]
    tempMatrix[2][2]=(temp[4]+temp[1])/2

    tempMatrix[3][0]=temp[2]
    tempMatrix[3][1]=temp[4]
    tempMatrix[3][2]=temp[4]

    tempMatrix[4][0]=temp[4]
    tempMatrix[4][1]=temp[4]
    tempMatrix[4][2]=temp[4]  
    print(tempMatrix)
    '''
    k=0
